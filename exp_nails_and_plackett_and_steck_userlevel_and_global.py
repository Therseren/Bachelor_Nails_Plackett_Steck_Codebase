...
# 12) Flat Top@N user-mean KL vs Exposure-weighted user-mean KL (user-level)
make_lambda_summary_plot_mean_only(
    df_eval_agg,
    x_col=LAM,
    series=[
        {"mean": "KL_topN_user_det_mean_mean_over_runs", "label": f"Flat user-mean KL (DET)"},
        {"mean": "KL_expw_det_user_mean_mean_over_runs", "label": f"Exposure user-mean KL (DET)"},
        {"mean": "KL_topN_user_pl_mean_mean_over_runs",  "label": f"Flat user-mean KL (PL)"},
        {"mean": "KL_expw_pl_user_mean_mean_over_runs",  "label": f"Exposure user-mean KL (PL)"},
        {"mean": "KL_topN_user_steck_mean_mean_over_runs", "label": f"Flat user-mean KL (STECK)"},
        {"mean": "KL_expw_steck_user_mean_mean_over_runs", "label": f"Exposure user-mean KL (STECK)"},
    ],
    title=f"Why exposure weighting matters: flat vs exposure KL (mean over users, Top@{topN})",
    ylabel="Smoothed KL",
    outpath=plot_dir.joinpath(f"summary_flat_vs_expw_KL_userMean_top{topN}_{PLOT_TAG}.png"),
)
